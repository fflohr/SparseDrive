...
class BEVRendererNode(Node):
    def __init__(self):
        ...
        self.create_subscription(Detection3DArray, '/detection', lambda msg: setattr(self, 'latest_det', msg), 10)
        self.create_subscription(MultiPath, '/traj', lambda msg: setattr(self, 'latest_traj', msg), 10)
        self.create_subscription(Path, '/plan', lambda msg: setattr(self, 'latest_plan', msg), 10)
        self.create_subscription(Path, '/plan_gt', lambda msg: setattr(self, 'latest_plan_gt', msg), 10)
        self.create_subscription(MarkerArray, '/map_pred', lambda msg: setattr(self, 'latest_map_pred', msg), 10)
        self.create_subscription(MarkerArray, '/map_gt', lambda msg: setattr(self, 'latest_map_gt', msg), 10)
    ...
    def draw_map(self, image, msg):
        for marker in msg.markers:
            points = np.array([self.world_to_pixel(p.x, p.y) for p in marker.points], dtype=np.int32)
            if len(points) > 1:
    ...
    def render_traj(self, image, traj, colormap, linestyle='-'):
        ...
        for i in range(len(traj) - 1):
            ...
            if linestyle == '-': cv2.line(image, tuple(traj[i]), tuple(traj[i+1]), color, 2)
            else: cv2.line(image, tuple(traj[i]), tuple(traj[i+1]), color, 1, cv2.LINE_AA)

    def render_sdc_car(self, image):
        try:
            car_path = os.path.join(get_package_share_directory('sparsedrive_ros2'), 'resource', 'sdc_car.png')
            ...
            x_offset, y_offset = int(self.image_width / 2 - w / 2), int(self.image_height / 2 - h / 2)
        ...
        except Exception as e: self.get_logger().warn(f"Error rendering SDC car: {e}")

    def render_legend(self, image):
        try:
            legend_path = os.path.join(get_package_share_directory('sparsedrive_ros2'), 'resource', 'legend.png')
            legend_png = cv2.imread(legend_path)
            h, w, _ = legend_png.shape
            image[self.image_height - h - 10:self.image_height - 10, self.image_width - w - 10:self.image_width - 10] = legend_png
        except Exception as e: self.get_logger().warn(f"Error rendering legend: {e}")
